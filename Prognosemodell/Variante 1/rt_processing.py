...
def get_rt_cv(train_set, val_set):
    ...
    train_X = train_set[['year', 'month', 'weekday', 'hour']].to_numpy()
    train_y = train_set[['cnt']].to_numpy()
    val_X = val_set[['year', 'month', 'weekday', 'hour']].to_numpy()
...
def get_rt_performance(train_set, val_set, test_set, min_leaf):
    ...
    val_X = val_set[['year', 'month', 'weekday', 'hour']].to_numpy()
    val_y = val_set[['cnt']].to_numpy()
    test_X = test_set[['year', 'month', 'weekday', 'hour']].to_numpy()
    ...
    tree.plot_tree(fitted_tree,
                feature_names = ['year', 'month', 'weekday', 'hour'], 
                max_depth=2,
                filled = True)
