...
def get_baseline_performance(train_set, val_set, test_set):
    ...
    # share Durchschnittswerte  für Train/Val/Test Daten
    train_set['mean_share'] = train_set['share'].mean()
    val_set['mean_share'] = val_set['share'].mean()
    test_set['mean_share'] = test_set['share'].mean()
    
    # Berechnung der Qualitätswerte
    R2_TR, RMSE_TR, MAE_TR, MAPE_TR = get_r2_rmse_mae_mape(train_set['share'].to_numpy(), train_set['mean_share'].to_numpy())
    R2_VAL, RMSE_VAL, MAE_VAL, MAPE_VAL = get_r2_rmse_mae_mape(val_set['share'].to_numpy(), val_set['mean_share'].to_numpy())
    R2_TE, RMSE_TE, MAE_TE, MAPE_TE = get_r2_rmse_mae_mape(test_set['share'].to_numpy(), test_set['mean_share'].to_numpy())
...
def get_rt_cv(train_set, val_set):
    ...
    train_X = train_set[['year', 'month', 'weekday', 'hour', 'station_nr']].to_numpy()
    train_y = train_set[['share']].to_numpy()
    val_X = val_set[['year', 'month', 'weekday', 'hour', 'station_nr']].to_numpy()
    val_y = val_set[['share']].to_numpy()
...
def get_rt_performance(train_set, val_set, test_set, min_leaf):
    ...
    val_X = val_set[['year', 'month', 'weekday', 'hour', 'station_nr']].to_numpy()
    val_y = val_set[['share']].to_numpy()
    test_X = test_set[['year', 'month', 'weekday', 'hour', 'station_nr']].to_numpy()
    test_y = test_set[['share']].to_numpy()
    ...
    R2_TR, RMSE_TR, MAE_TR, MAPE_TR = get_r2_rmse_mae_mape(train_set['share'].to_numpy(), train_y_pred)
    R2_VAL, RMSE_VAL, MAE_VAL, MAPE_VAL = get_r2_rmse_mae_mape(val_set['share'].to_numpy(), val_y_pred)
    R2_TE, RMSE_TE, MAE_TE, MAPE_TE = get_r2_rmse_mae_mape(test_set['share'].to_numpy(), test_y_pred)
